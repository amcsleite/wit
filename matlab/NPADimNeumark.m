function [out] = dimNeumark(witness, dimension)
rng('shuffle', 'twister');
clc;
...
constraints = [];
    ...
    function mkBasis()
        ...
        while true
            ...
            [pTerms, mTerms] = randomNeumark(field, preparations, measurements, outcomes, dimension);
...
%permutations = perms(1:outcomes);
%permutations = combnk(1 : 3, 2);
permutations = [1 : outcomes];

[nPerms,~] = size(permutations);

for per = 1 : nPerms
    objF = mkObjF1(permutations(per, :));
    errorCode = optimize(constraints, -real(objF), options);
    
    result(per) = value(objF);
    mxs{per} = value(mM);
